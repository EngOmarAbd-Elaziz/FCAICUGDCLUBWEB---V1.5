...
export function SeasonsSection() {
  ...
  const handleDelete = (id: string) => {
    showConfirmModal('Are you sure? This will also delete all team members and waves linked to this season.', async () => {
      try {
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    setIsSaving(true);
    try {
      const payload = { name, display_order: displayOrder };
      if (currentItem?.id) {
        const { error } = await supabase.from('seasons').update(payload).eq('id', currentItem.id);
        if (error) throw error;
      } else {
  ...
  return (
    <div className="admin-section">
      ...
      <div>
        {items.length === 0 ? <p>No seasons found.</p> : items.map(item => (
          <div className="item-card" key={item.id}>
            <div className="item-info">
              <div className="item-title">{item.name}</div>
              <div className="item-subtitle">Order: {item.display_order}</div>
            </div>
      ...
      <AdminModal isOpen={isModalOpen} onClose={() => setIsModalOpen(false)} title={currentItem ? 'Edit Season' : 'Add New Season'} onSubmit={handleSubmit} isSaving={isSaving}>
        <div className="form-group"><label>Season Name:</label><input type="text" value={name} onChange={e => setName(e.target.value)} required placeholder="e.g. Season 1 (2023-2024)" /></div>
